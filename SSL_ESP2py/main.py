...
mqtt_port = 8883      # Port MQTT sécurisé (SSL/TLS)
...
def connect_to_mqtt():
    client = MQTTClient(client_id, mqtt_server, port=mqtt_port, ssl=True)
    client.set_callback(mqtt_callback)

    while True:
        try:
            print("Connexion au broker MQTT sécurisé...")
            client.connect()
            print("Connecté au broker MQTT sécurisé")
